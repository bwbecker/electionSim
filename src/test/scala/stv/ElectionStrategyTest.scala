...
object ElectionStrategyTest extends TestSuite {
  ...
  val c = Seq(
    ...
    Candidate("1", "R1", AB, "C1", Lib, 200, 200, false, RidingSeat, 0),
    Candidate("1", "R1", AB, "C2", NDP, 150, 150, false, RidingSeat, 0)
  )

  val c2 = c :+ Candidate("1", "R1", AB, "C3", Con, 150, 150, false, RidingSeat, 0)
  ...
  val tests = this {
    ...
    "FptpRidingElectionStrategy:" - {
      ...
      "on candidates with distinct parties" - {
        ...
        assert(elected == List(Candidate("1", "R1", AB, "C1", Lib, 200, 200, true, RidingSeat, 0)))
      ...
      "on candidates with same party" - {
        ...
        assert(elected == List(Candidate("1", "R1", AB, "C3", Con, 150, 250, true, RidingSeat, 0)))
    ...
    "StvRidingElectionStrategy:" - {
      ...
      "on candidates with distinct parties" - {
        ...
        "one candidate right at quota" - {
          val candidates = c :+ Candidate("1", "R1", AB, "C3", Grn, 452, 452, false, RidingSeat, 0)
          ...
          assert(elected.head == Candidate("1", "R1", AB, "C3", Grn, 452, quota, true, RidingSeat, 0))
          assert(unelected.length == 3)
        ...
        "one candidate just under quota" - {
          val candidates = c :+ Candidate("1", "R1", AB, "C3", Grn, 450, 450, false, RidingSeat, 0)
          val quota = candidates.foldLeft(0.0)(_ + _.votes) / (1 + 1) + 1
          assert(quota == 451)
          val (elected, unelected) = stvStrat.runElection(candidates, 1)
          assert(elected.length == 1)
          assert(elected.head == Candidate("1", "R1", AB, "C3", Grn, 450, quota, true, RidingSeat, 1))

          assert(unelected.length == 3)

          assert(unelected(0) == Candidate("1", "R1", AB, "C0", Con, 100, 0, false, RidingSeat, 0))
          // 25 votes to Lib; 10 to Grn; none to NDP

          assert(unelected(1) == Candidate("1", "R1", AB, "C1", Lib, 200, 225, false, RidingSeat, 2))
          assert(unelected(2) == Candidate("1", "R1", AB, "C2", NDP, 150, 150, false, RidingSeat, 2))

        }

        "winner needs multiple transfers to make quota" - {
          val candidates = c :+ Candidate("1", "R1", AB, "C3", Grn, 148, 148, false, RidingSeat, 0)
          val quota = candidates.foldLeft(0.0)(_ + _.votes) / (1 + 1) + 1
          assert(quota == 300)
          val (elected, unelected) = stvStrat.runElection(candidates, 1)
          assert(elected.length == 1)
          assert(elected.head == Candidate("1", "R1", AB, "C1", Lib, 200, quota, true, RidingSeat, 2))

          assert(unelected.length == 3)
          val u = unelected.sortBy(_.name)
          // Cons out.  25 to Lib; 10 to Grn.  NDP out.  75 to Lib.
          assert(u(0) == Candidate("1", "R1", AB, "C0", Con, 100, 0, false, RidingSeat, 0))
          assert(u(1) == Candidate("1", "R1", AB, "C2", NDP, 150, 0, false, RidingSeat, 1))
          assert(u(2) == Candidate("1", "R1", AB, "C3", Grn, 148, 158, false, RidingSeat, 3))
      ...
      "winner's excess redistributed" - {
        val candidates = Seq(
          Candidate("1", "R1", AB, "C0", Con, 100, 100, false, RidingSeat, 0),
          Candidate("1", "R1", AB, "C1", Lib, 350, 350, false, RidingSeat, 0),
          Candidate("1", "R1", AB, "C2", NDP, 150, 150, false, RidingSeat, 0),
          Candidate("1", "R1", AB, "C3", Grn, 198, 198, false, RidingSeat, 0)
        )
        ...
        val (elected, unelected) = stvStrat.runElection(candidates, 1)
        assert(elected.length == 1)
        assert(elected.head == Candidate("1", "R1", AB, "C1", Lib, 350, quota, true, RidingSeat, 2))
        ...
        assert(u(0) == Candidate("1", "R1", AB, "C0", Con, 100, 0, false, RidingSeat, 0))
        assert(u(1) == Candidate("1", "R1", AB, "C2", NDP, 150, 0, false, RidingSeat, 1))
        assert(u(2) == Candidate("1", "R1", AB, "C3", Grn, 198, 213, false, RidingSeat, 3))
